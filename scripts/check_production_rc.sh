...
[[ "${matrix_rows}" == 36 ]] || fail "expected 36 coverage rows, found ${matrix_rows}"
...
[[ "${implemented}" == 33 && "${partial}" == 3 && "${open}" == 0 ]] || \
  fail "PR99 coverage must be 33 implemented, 3 partial and 0 open (found ${implemented}/${partial}/${open})"
grep -Fq $'TST027\tproduction release-candidate gate\timplemented' "${MATRIX_FILE}" || \
  fail "TST027 must be implemented by the PR99 gate"
for number in $(seq 1 36); do grep -Fq "TST$(printf '%03d' "${number}")" "${MATRIX_FILE}" || fail "missing TST row ${number}"; done

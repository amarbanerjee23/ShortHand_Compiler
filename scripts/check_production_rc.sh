...
[[ "${matrix_rows}" == 39 ]] || fail "expected 39 coverage rows, found ${matrix_rows}"
...
[[ "${implemented}" == 36 && "${partial}" == 3 && "${open}" == 0 ]] || \
  fail "PR102 coverage must be 36 implemented, 3 partial and 0 open (found ${implemented}/${partial}/${open})"
grep -Fq $'TST027\tproduction release-candidate gate\timplemented' "${MATRIX_FILE}" || \
  fail "TST027 must be implemented by the PR99 gate"
for number in $(seq 1 39); do grep -Fq "TST$(printf '%03d' "${number}")" "${MATRIX_FILE}" || fail "missing TST row ${number}"; done

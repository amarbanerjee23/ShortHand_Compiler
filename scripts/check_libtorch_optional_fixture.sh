...
require_contains() {
  ...
  if ! grep -Fq "${needle}" "${file}"; then
    echo "error: ${file} missing required LibTorch fixture text: ${needle}" >&2
    exit 1
  fi
}
...
require_file Compiler_new_ws/Short_Hand/src/ai_runtime/backends/LibTorchBackend.cpp
...
require_contains docs/libtorch_optional_fixture.md 'shorthand.backend_libtorch_optional_fixture.v1'
...
require_contains Compiler_new_ws/Short_Hand/src/ai_runtime/backends/LibTorchBackend.cpp 'LibTorch SDK detected but direct execution is not yet enabled'
require_contains tests/integration/test_libtorch_optional_fixture.sh 'SHORTHAND_RUNTIME_ENABLE_AI_RUNTIME_BRIDGE=1'
require_contains tests/integration/test_libtorch_optional_fixture.sh 'SHORTHAND_HAS_LIBTORCH=${LIBTORCH_MACRO}'
require_contains tests/integration/test_libtorch_optional_fixture.sh 'short_ai_infer_f32("libtorch_model"'
require_contains tests/integration/test_libtorch_optional_fixture.sh 'unavailable_path_proved'
...
bash tests/integration/test_libtorch_optional_fixture.sh

echo "PASS LibTorch optional fixture gate"

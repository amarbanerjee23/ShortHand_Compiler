...
require_file_pattern "${DOCKERFILE}" 'shorthand_serving_worker.*probe.*--live' 'healthcheck must execute serving liveness'
...
require_file_pattern "${DOCKERFILE}" 'serving_worker' 'serving worker must be built in builder stage'
require_file_pattern "${DOCKERFILE}" '--max-in-flight-request-bytes.*67108864' 'aggregate in-flight request byte limit'
require_file_pattern "${DOCKERFILE}" '--max-retained-result-bytes.*67108864' 'aggregate retained-result byte limit'
...
require_file_pattern "${MANIFEST}" 'shorthand_serving_worker' 'serving worker process and probes'
require_file_pattern "${MANIFEST}" '/tmp/shorthand-serving-health\.json' 'bounded serving health state'
require_file_pattern "${MANIFEST}" '--ready' 'readiness closes for saturation and drain'
require_file_pattern "${MANIFEST}" '--live' 'liveness remains separate from readiness'
require_file_pattern "${MANIFEST}" 'kill -USR1 1' 'preStop initiates serving drain'
require_file_pattern "${MANIFEST}" '--max-in-flight-request-bytes' 'aggregate in-flight request byte limit'
require_file_pattern "${MANIFEST}" '--max-retained-result-bytes' 'aggregate retained-result byte limit'
...
printf 'PASS container Kubernetes production hardening contract multi_stage=true non_root=true probes=3 bounded_serving_memory=true bounded_workspace=true default_deny=true quota=true pdb=true graceful_drain=true\n'

...
require_contains() {
  ...
  if ! grep -Fq "${needle}" "${file}"; then
    echo "error: ${file} missing required TensorRT fixture text: ${needle}" >&2
    exit 1
  fi
}
...
require_file Compiler_new_ws/Short_Hand/src/ai_runtime/backends/TensorRTBackend.cpp
...
require_contains tests/integration/test_tensorrt_optional_fixture.sh 'SHORTHAND_HAS_TENSORRT=${TENSORRT_MACRO}'
require_contains tests/integration/test_tensorrt_optional_fixture.sh 'short_ai_infer_f32("trt_model"'
require_contains tests/integration/test_tensorrt_optional_fixture.sh 'unavailable_path_proved'
...
bash tests/integration/test_tensorrt_optional_fixture.sh

echo "PASS TensorRT optional fixture gate"

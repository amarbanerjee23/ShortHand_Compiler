...
for file in \
  ...
  "${GRAMMAR_DOC}" "${SPEC_DOC}" "${MODULE_DOC}" "${TRACKER}" \
  "${BASE_GATE}" "${MODULE_GATE}"; do
  require_file "${file}"
done
...
require_contains "${MODULE_DOC}" 'multi_file_execution_status: deferred_to_pr70'
...
require_manifest_rows_have_four_fields
for category in current-version version grammar-matrix grammar-extension parser-valid parser-invalid semantic-invalid diagnostics codegen runtime evidence; do

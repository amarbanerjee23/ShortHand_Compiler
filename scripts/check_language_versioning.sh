...
for file in \
  "${MANIFEST}" "${BASE_MATRIX}" "${MODULE_MATRIX}" "${TYPE_MATRIX}" "${CONTROL_MATRIX}" "${ENTERPRISE_MATRIX}" "${VERSION_DOC}" \
  "${GRAMMAR_DOC}" "${SPEC_DOC}" "${MODULE_DOC}" "${RESOLVER_DOC}" "${TRACKER}" \
  "${CONTROL_DOC}" "${ENTERPRISE_DOC}" "${BASE_GATE}" "${MODULE_GATE}" "${RESOLVER_GATE}" "${TYPE_GATE}" "${CONTROL_GATE}" "${ENTERPRISE_GATE}"; do
  require_file "${file}"
done

require_contains "${VERSION_DOC}" 'shorthand.language.version: beta-0.6'
require_contains "${VERSION_DOC}" 'shorthand.conformance.contract: beta-0.6'
require_contains "${VERSION_DOC}" 'shorthand.grammar.matrix: beta-0.2-base+beta-0.3-modules+beta-0.4-types+beta-0.5-control+beta-0.6-enterprise'
...
require_contains "${SPEC_DOC}" 'Language version: beta-0.6'
...
require_contains "${MANIFEST}" 'current-version | shorthand.language.version | beta-0.6 | Current layered language contract marker.'
...
require_manifest_rows_have_four_fields
for category in current-version version grammar-matrix grammar-extension type-matrix control-flow-matrix enterprise-matrix module-resolution parser-valid parser-invalid semantic-invalid diagnostics codegen runtime typed-runtime evidence; do
...
printf 'PASS language versioning and conformance gate beta-0.6\n'

...
require_file "${PLAN}"
require_contains "${PLAN}" 'production_readiness_plan_version: 2026-07-27-pr54'
require_contains "${PLAN}" 'PLAN_STATUS: active'
require_contains "${PLAN}" 'LAST_COMPLETED_PR: 54'
...
require_contains "${PLAN}" 'Next recommended PR after PR #54:'
...
require_contains "${PLAN}" 'docs/openvino_optional_fixture.md'
require_contains "${PLAN}" 'scripts/check_openvino_optional_fixture.sh'
require_contains "${PLAN}" 'The OpenVINO row has an explicit unavailable-path proof'
require_contains "${PLAN}" 'PR55 - LibTorch optional live execution fixture'
...
require_contains "${PLAN}" 'remaining_planned_prs_after_pr54: 24'

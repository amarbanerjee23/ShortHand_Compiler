...
require_contains "${PLAN}" 'production_readiness_plan_version: 2026-08-02-pr58'
require_contains "${PLAN}" 'PLAN_STATUS: active'
require_contains "${PLAN}" 'LAST_COMPLETED_PR: 58'
...
require_contains "${PLAN}" 'Next recommended PR after PR #58:'
...
require_contains "${PLAN}" 'tests/integration/test_backend_failure_mode_matrix.sh'
require_contains "${PLAN}" 'scripts/check_backend_failure_mode_matrix.sh'
require_contains "${PLAN}" 'PR59 - Runtime ABI and API version stability gate'
...
require_contains "${PLAN}" 'remaining_planned_prs_after_pr58: 21'

...
require_file "${PLAN}"
require_contains "${PLAN}" 'production_readiness_plan_version: 2026-07-28-pr55'
require_contains "${PLAN}" 'PLAN_STATUS: active'
require_contains "${PLAN}" 'LAST_COMPLETED_PR: 55'
...
require_contains "${PLAN}" 'Detect CPU capability and available accelerator classes such as GPU, TPU, and NPU'
require_contains "${PLAN}" 'Separate `detected`, `accessible`, `backend_compatible`, and `execution_ready` states.'
...
require_contains "${PLAN}" 'Next recommended PR after PR #55:'
require_contains "${PLAN}" 'PR56 - Hardware capability discovery and accelerator-aware routing.'
...
require_contains "${PLAN}" 'docs/libtorch_optional_fixture.md'
require_contains "${PLAN}" 'scripts/check_libtorch_optional_fixture.sh'
require_contains "${PLAN}" 'The LibTorch row has an explicit unavailable-path proof'
require_contains "${PLAN}" 'PR56 - Hardware capability discovery and accelerator-aware routing'
require_contains "${PLAN}" 'PR57 - Llama.cpp optional live execution fixture'
require_contains "${PLAN}" 'PR58 - Backend failure-mode matrix finalization'
require_contains "${PLAN}" 'PR59 - Runtime ABI and API version stability gate'
...
require_contains "${PLAN}" 'Hardware inventory distinguishes detected, accessible, backend-compatible, and execution-ready devices.'
require_contains "${PLAN}" 'Production claim rules'
require_contains "${PLAN}" 'remaining_planned_prs_total_from_pr51: 29'
require_contains "${PLAN}" 'remaining_planned_prs_after_pr55: 24'

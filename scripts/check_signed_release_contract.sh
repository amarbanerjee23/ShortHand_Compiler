...
required_files=(
  "${WORKFLOW}"
  scripts/check_release_version_policy.sh
...
for script in scripts/check_release_version_policy.sh scripts/prepare_release_bundle.sh scripts/verify_release_bundle.sh scripts/check_protected_release_environment.sh tests/release/*.sh; do
  bash -n "${script}"
done

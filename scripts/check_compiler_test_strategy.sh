...
[[ "${row_count}" == 39 ]] || { echo "error: expected 39 compiler test coverage rows, found ${row_count}" >&2; exit 1; }
[[ "${implemented_count}" == 36 ]] || { echo "error: expected 36 implemented rows in the PR102 candidate" >&2; exit 1; }
[[ "${partial_count}" == 3 ]] || { echo "error: expected 3 partial rows in the PR102 candidate" >&2; exit 1; }
[[ "${open_count}" == 0 ]] || { echo "error: expected 0 open rows in the PR102 candidate" >&2; exit 1; }
...
require_contains "${STATUS}" 'feature_status_version: 2026-09-18-pr102'
require_contains "${STATUS}" '36 implemented, 3 partial and 0 open'

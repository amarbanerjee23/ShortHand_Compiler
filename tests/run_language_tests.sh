...
run_ai(){ if [[ -x "${BUILD_DIR}/short_ai_app" ]]; then "${BUILD_DIR}/short_ai_app" missing.onnx "1" "0" >/tmp/shorthand_ai.out 2>&1 || true; grep -Eiq 'onnx|unavailable|missing|error' /tmp/shorthand_ai.out && ok ai_fallback || bad ai_fallback; else warning "short_ai_app unavailable; AI tests environment-limited"; fi; }
case "$MODE" in
  unit) run_green ;;

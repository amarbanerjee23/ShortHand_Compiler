...
TENSORRT_MACRO=0
TENSORRT_REASON="TENSORRT_ROOT_not_set"
if [[ -n "${TENSORRT_ROOT:-}" ]]; then
  TENSORRT_MACRO=1
  TENSORRT_REASON="TENSORRT_ROOT_set_but_direct_execution_not_enabled"
fi

cat > "${WORK_DIR}/tensorrt_optional_fixture_probe.cpp" <<'CPP'
...
int main() {
    ...
    if (short_ai_register_model("trt_model", "engine", "models/not_a_real_fixture.engine", "identity", "float32", "1", "1", "tensorrt") != SHORTHAND_RUNTIME_OK) return 4;

    float input_values[1] = {42.0f};
    float output_values[1] = {-999.0f};
    int output_count = -1;

    const int status = short_ai_infer_f32("trt_model", "input", input_values, 1, "output", output_values, 1, &output_count);

    if (status == SHORTHAND_RUNTIME_OK) {
        std::cerr << "error: TensorRT unavailable-path fixture returned success\n";
        return 10;
    }
    if (status != SHORTHAND_RUNTIME_BACKEND_UNAVAILABLE && status != SHORTHAND_RUNTIME_NOT_EXECUTED) {
        std::cerr << "error: unexpected TensorRT unavailable-path status=" << status << "\n";
        return 11;
    }
    if (short_runtime_infer_success_count() != 0) {
        std::cerr << "error: TensorRT unavailable-path fixture incremented success counter\n";
        return 12;
    }
    if (output_count > 0) {
        std::cerr << "error: TensorRT unavailable-path fixture copied outputs\n";
        return 13;
    }
    if (output_values[0] != -999.0f) {
        std::cerr << "error: TensorRT unavailable-path fixture mutated output buffer\n";
        return 14;
    }
    ...
    std::cout << "TENSORRT_FIXTURE backend=tensorrt status=unavailable_path_proved reason="
              << short_runtime_last_infer_reason() << " runtime_status=" << status << "\n";
    std::cout << "PASS tensorrt optional fixture gate\n";
    return 0;
}
CPP
...
${CXX:-g++} -std=c++17 -Wall -Wextra -Wpedantic \
  ...
  -DSHORTHAND_HAS_TENSORRT=${TENSORRT_MACRO} \
  ...
  -I"${SRC_DIR}" \
  "${WORK_DIR}/tensorrt_optional_fixture_probe.cpp" \
  ...
  -o "${WORK_DIR}/tensorrt_optional_fixture_probe"

"${WORK_DIR}/tensorrt_optional_fixture_probe" \
  >/tmp/shorthand_tensorrt_optional_fixture.out \
  2>/tmp/shorthand_tensorrt_optional_fixture.err

grep -q 'PASS tensorrt optional fixture gate' /tmp/shorthand_tensorrt_optional_fixture.out
if grep -qi 'status=success\|SHORTHAND_RUNTIME_OK\|live_success' /tmp/shorthand_tensorrt_optional_fixture.out /tmp/shorthand_tensorrt_optional_fixture.err; then
  echo "error: TensorRT optional fixture made a false success claim" >&2
  cat /tmp/shorthand_tensorrt_optional_fixture.out >&2 || true
  cat /tmp/shorthand_tensorrt_optional_fixture.err >&2 || true
  exit 1
fi

cat /tmp/shorthand_tensorrt_optional_fixture.out
printf 'TENSORRT_FIXTURE_ENV %s\n' "${TENSORRT_REASON}"

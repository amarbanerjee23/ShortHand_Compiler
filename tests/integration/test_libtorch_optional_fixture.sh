...
LIBTORCH_MACRO=0
LIBTORCH_REASON="LIBTORCH_ROOT_not_set"
if [[ -n "${LIBTORCH_ROOT:-}" ]]; then
  LIBTORCH_MACRO=1
  LIBTORCH_REASON="LIBTORCH_ROOT_set_but_direct_execution_not_enabled"
fi

cat > "${WORK_DIR}/libtorch_optional_fixture_probe.cpp" <<'CPP'
...
int main() {
    ...
    if (short_ai_register_model("libtorch_model", "torchscript", "models/not_a_real_fixture.pt", "identity", "float32", "1", "1", "libtorch") != SHORTHAND_RUNTIME_OK) return 4;

    float input_values[1] = {42.0f};
    float output_values[1] = {-999.0f};
    int output_count = -1;

    const int status = short_ai_infer_f32("libtorch_model", "input", input_values, 1, "output", output_values, 1, &output_count);

    if (status == SHORTHAND_RUNTIME_OK) {
        std::cerr << "error: LibTorch unavailable-path fixture returned success\n";
        return 10;
    }
    if (status != SHORTHAND_RUNTIME_BACKEND_UNAVAILABLE && status != SHORTHAND_RUNTIME_NOT_EXECUTED) {
        std::cerr << "error: unexpected LibTorch unavailable-path status=" << status << "\n";
        return 11;
    }
    if (short_runtime_infer_success_count() != 0) {
        std::cerr << "error: LibTorch unavailable-path fixture incremented success counter\n";
        return 12;
    }
    if (output_count > 0) {
        std::cerr << "error: LibTorch unavailable-path fixture copied outputs\n";
        return 13;
    }
    if (output_values[0] != -999.0f) {
        std::cerr << "error: LibTorch unavailable-path fixture mutated output buffer\n";
        return 14;
    }
    ...
    std::cout << "LIBTORCH_FIXTURE backend=libtorch status=unavailable_path_proved reason="
              << short_runtime_last_infer_reason() << " runtime_status=" << status << "\n";
    std::cout << "PASS libtorch optional fixture gate\n";
    return 0;
}
CPP
...
${CXX:-g++} -std=c++17 -Wall -Wextra -Wpedantic \
  ...
  -DSHORTHAND_HAS_LIBTORCH=${LIBTORCH_MACRO} \
  -DSHORTHAND_HAS_LLAMACPP=0 \
  -DSHORTHAND_HAS_OPENBLAS=0 \
  -DSHORTHAND_HAS_EIGEN=0 \
  -I"${SRC_DIR}" \
  "${WORK_DIR}/libtorch_optional_fixture_probe.cpp" \
  ...
  -o "${WORK_DIR}/libtorch_optional_fixture_probe"

"${WORK_DIR}/libtorch_optional_fixture_probe" \
  >/tmp/shorthand_libtorch_optional_fixture.out \
  2>/tmp/shorthand_libtorch_optional_fixture.err

grep -q 'PASS libtorch optional fixture gate' /tmp/shorthand_libtorch_optional_fixture.out
if grep -qi 'status=success\|SHORTHAND_RUNTIME_OK\|live_success' /tmp/shorthand_libtorch_optional_fixture.out /tmp/shorthand_libtorch_optional_fixture.err; then
  echo "error: LibTorch optional fixture made a false success claim" >&2
  cat /tmp/shorthand_libtorch_optional_fixture.out >&2 || true
  cat /tmp/shorthand_libtorch_optional_fixture.err >&2 || true
  exit 1
fi

cat /tmp/shorthand_libtorch_optional_fixture.out
printf 'LIBTORCH_FIXTURE_ENV %s\n' "${LIBTORCH_REASON}"

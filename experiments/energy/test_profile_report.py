...
class ProfileTests(unittest.TestCase):
    ...
    def test_nested_partition(self):
        report = fixture(); report['schema'] = 'shorthand.ai.application.profile.v2'
        trial = report['trials'][0]
        trial['backend'] = dict(setup_ns=4, input_validation_ns=5, tensor_setup_ns=6,
                                session_run_ns=20, output_copy_ns=5, telemetry_ns=10, total_ns=50)
        trial['prepared_call_unattributed_ns'] = 10
        self.assertEqual(validate(report)['prepared_call_ns'], 60)
        self.assertIn('33.33%', markdown(report, '0' * 64))
        self.assertIn('serialization', markdown(report, '0' * 64))
        for key, value in [('total_ns', 49), ('telemetry_ns', -1), ('session_run_ns', True)]:
            bad = copy.deepcopy(report); bad['trials'][0]['backend'][key] = value
            with self.subTest(key=key), self.assertRaises(ValueError): validate(bad)
        for value in (-1, 11, True):
            bad = copy.deepcopy(report); bad['trials'][0]['prepared_call_unattributed_ns'] = value
            with self.assertRaises(ValueError): validate(bad)
        del trial['backend']['setup_ns']

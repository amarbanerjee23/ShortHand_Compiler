...
| --- | --- | --- | --- |
| ONNX | `onnxruntime_cpu`, `onnxruntime_cuda`, `onnxruntime_tensorrt`, `tensorrt` | Yes, but fallback must report `not_executed` | `onnxruntime_cpu` has SDK-backed execution when `ONNXRUNTIME_ROOT` is configured; PR #52 adds a live SDK matrix harness for row-level status reporting |
| TensorRT engine | `tensorrt`, `onnxruntime_tensorrt` | Yes, but fallback must report `not_executed` | Policy-compatible only until PR #53 adds an optional live fixture or explicit unavailable-path proof |
...
| `backend_live_sdk_matrix_harness` | One matrix runner records a row for each marketed backend as `live_success`, `skip_safe`, `policy_compatible_only`, or `dedicated_fixture_planned`. | `tests/integration/test_backend_live_sdk_matrix.sh` writes `/tmp/shorthand_backend_live_sdk_matrix.jsonl` and runs the ONNX Runtime compiled-hook success fixture when configured. | Only rows with real fixture execution may claim `live_success`. |
...
PR #52 adds the first shared live SDK matrix harness.
...
The harness must keep default CI skip-safe. It may report `live_success` only for `onnxruntime_cpu` when `ONNXRUNTIME_ROOT` is configured and the compiled-hook ONNX Runtime success fixture passes. All other marketed backend rows remain `skip_safe`, `policy_compatible_only`, or `dedicated_fixture_planned` until their dedicated PRs add real fixtures or explicitly remove production support claims.

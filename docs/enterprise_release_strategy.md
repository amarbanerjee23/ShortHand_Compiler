...
GitHub PR94 implements original roadmap PR93 in the Linux x64/LLVM18 scope: verified SemanticIR, source and SDK lowering, bounded composite values, checked real ONNX runtime calls and optimization-preserved evidence. See [the lowering contract](mlir_lowering.md). TST024 is implemented within this scope. Merged GitHub PR93 is the separate gap assessment; PR101 is merged and PR102 is the final planned implementation batch; calibrated physical data, independently reviewed organizational operations and the protected release exercise still govern eligibility.

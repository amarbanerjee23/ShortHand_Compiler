...
| L5 | Emit runtime metadata for AI declarations and infer in compiled code instead of no-op lowering | Partial | `IR_Generator.cpp` emits LLVM metadata globals and compiled runtime hook calls; `runtime/ShorthandRuntime.cpp` provides a linkable hook library; `scripts/generate_external_runtime_ir_generator.sh` creates an external-runtime lowering variant; `tests/codegen/test_external_runtime_native.sh` verifies external declarations and native runtime-link execution | Demonstrates native AI/GreenAI hook execution through `libshorthand_runtime.a`; next step is to make the external-runtime lowering path the default source implementation instead of a generated transitional path |
...
| C3L-5 | Resource and telemetry capture primitives | Partial | ONNX Runtime path now emits latency/input/output telemetry JSON; external runtime native linking gate exists; direct energy measurement and OTLP SDK export remain open |
...
| C3L-12 | CI/CD and eco-regression gates | Partial | CI exercises candidate report/check/workbook paths, enterprise hardening checks, source diagnostics, runtime-library build checks, external runtime native linking, and the optional ONNX SDK gate skip path; release-to-release eco-regression is still open |
...
| R3 | AI runtime behavior | Partial | Fallback is honest; ONNX Runtime CPU execution exists for SDK-enabled builds with an optional model-fixture gate; `libshorthand_runtime.a` exports hook symbols; generated external-runtime lowering is verified through a native-link test path |
...
| G14-G16 GreenAI measurement/evidence | Partial | Evidence modes and candidate bundle generator exist; full measurement plan and authority-ready bundle remain open |
...
2. Compiled-code metadata/runtime lowering for `model`, `tensor`, `greenai_contract`, `greenai_measure`, and `infer`: metadata globals, runtime hook calls, runtime hook library, and an external-runtime native-link test path exist; remaining work is to replace the transitional generated external-runtime lowering with direct source-level external hook declarations in `IR_Generator.cpp`.

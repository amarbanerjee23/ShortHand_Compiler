...
| G11 real AI backend execution | Partial | Real ONNX Runtime CPU execution is implemented behind `ONNXRUNTIME_ROOT`; optional SDK gate with small ONNX fixture exists and skips safely when SDK is absent; runtime hook observability, typed buffer bridge request scaffolding, backend matrix gate, linkage guardrails, AI runtime adapter mapping, and bridge link-build validation exist, but compiled-hook SDK execution remains open |

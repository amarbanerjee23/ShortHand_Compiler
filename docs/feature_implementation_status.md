...
| L5 | Emit runtime metadata for AI declarations and infer in compiled code instead of no-op lowering | Partial | `IR_Generator.cpp` emits LLVM metadata globals and compiled runtime hook calls; `scripts/generate_external_runtime_ir_generator.sh` generates the default external-runtime source used by Makefile/CMake; `runtime/ShorthandRuntime.cpp` provides model/tensor/contract/measurement registries with status codes; `tests/codegen/test_external_runtime_native.sh` verifies default compiler external declarations and native runtime-link execution | Default builds now generate external runtime hook declarations and link native binaries through `libshorthand_runtime.a`; remaining work is direct source-level cleanup in `IR_Generator.cpp` and routing `infer` into `AI_Runtime` |
...
| C3L-5 | Resource and telemetry capture primitives | Partial | ONNX Runtime path now emits latency/input/output telemetry JSON; default external runtime native linking and registry-backed hook state exist; semantic IR measurement scaffolding exists; direct energy measurement and OTLP SDK export remain open |
...
| C3L-12 | CI/CD and eco-regression gates | Partial | CI exercises candidate report/check/workbook paths, enterprise hardening checks, source diagnostics, language conformance, runtime-library build checks, default external runtime native linking, registry-backed runtime hook checks, and the optional ONNX SDK gate skip path; release-to-release eco-regression is still open |
...
| R3 | AI runtime behavior | Partial | Fallback is honest; ONNX Runtime CPU execution exists for SDK-enabled builds with an optional model-fixture gate; `libshorthand_runtime.a` exports hook symbols and maintains model/tensor/contract/measurement registries; default Makefile/CMake builds generate external runtime hook declarations and validate native linking |
| R4 | GreenAI and C3-ECO-aligned evidence | Partial | Evidence/report modes, semantic IR measurement scaffolding, and bundle generator exist; full authority-ready evidence schema remains open |
...
| G14-G16 GreenAI measurement/evidence | Partial | Evidence modes, candidate bundle generator, semantic IR measurement op, and runtime measurement registry exist; full measurement plan and authority-ready bundle remain open |
...
2. Compiled-code metadata/runtime lowering for `model`, `tensor`, `greenai_contract`, `greenai_measure`, and `infer`: metadata globals, runtime hook calls, runtime hook library, registry-backed hook state, default generated external-runtime lowering, and a default native-link test path exist; remaining work is direct source-level cleanup in `IR_Generator.cpp` and routing compiled infer into `AI_Runtime`.

...
| C3L-5 | Resource and telemetry capture primitives | Partial | ONNX Runtime path emits latency/input/output telemetry JSON; source-level external runtime native linking, registry-backed hook state, runtime infer counters, and observability JSON exist; direct energy measurement and OTLP SDK export remain open |
...
| C3L-12 | CI/CD and eco-regression gates | Partial | CI exercises candidate report/check/workbook paths, C3-ECO schema/claim-safety gate, enterprise hardening checks, source diagnostics, language conformance, runtime-library build checks, source-level external runtime native linking, registry-backed runtime hook checks, runtime infer observability checks, MLIR foundation checks, release supply-chain checks, and the optional ONNX SDK gate skip path; release-to-release eco-regression is still open |
...
2. Compiled-code metadata/runtime lowering for `model`, `tensor`, `greenai_contract`, `greenai_measure`, and `infer`: metadata globals, runtime hook calls, runtime hook library, registry-backed hook state, source-level external runtime lowering, runtime infer observability, and a default native-link test path exist; remaining work is routing compiled infer into `AI_Runtime` for SDK-backed execution.
...
6. Release supply-chain automation: candidate SBOM, source hashes, provenance JSON, and baseline scan exist; signed release artifacts and protected release publication remain open.

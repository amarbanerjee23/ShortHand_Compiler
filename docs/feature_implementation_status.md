...
| C3L-5 | Resource and telemetry capture primitives | Partial | ONNX Runtime path emits latency/input/output telemetry JSON; source-level external runtime native linking, registry-backed hook state, runtime infer counters, compiled infer bridge request JSON, and observability JSON exist; direct energy measurement and OTLP SDK export remain open |
...
| C3L-12 | CI/CD and eco-regression gates | Partial | CI exercises candidate report/check/workbook paths, C3-ECO schema/claim-safety gate, enterprise hardening checks, source diagnostics, language conformance, runtime-library build checks, source-level external runtime native linking, registry-backed runtime hook checks, runtime infer observability checks, compiled infer bridge request checks, MLIR foundation checks, release supply-chain checks, PR task-stability checks, and the optional ONNX SDK gate skip path; release-to-release eco-regression is still open |
...
| R3 | AI runtime behavior | Partial | Fallback is honest; ONNX Runtime CPU execution exists for SDK-enabled builds with an optional model-fixture gate; `libshorthand_runtime.a` exports hook symbols and maintains model/tensor/contract/measurement registries plus infer counters, last-infer status/reason/backend, telemetry JSON, observability JSON, and compiled infer bridge request JSON; real compiled-hook execution through `AI_Runtime` remains open until typed tensor buffers are available |
...
| R7 | Deployment and operations | Partial | Docker/Kubernetes scaffolds, observability plan, runtime hook library, runtime telemetry JSON fragments, runtime observability JSON, and compiled infer bridge request JSON exist; production OTLP/Prometheus export remains open |
| R8 | Governance and release control | Partial | Release plans/status docs, candidate provenance output, and PR task-stability guardrails exist; full RFC workflow, protected release workflow, and signed release provenance are open |
...
| G11 real AI backend execution | Partial | Real ONNX Runtime CPU execution is implemented behind `ONNXRUNTIME_ROOT`; optional SDK gate with small ONNX fixture exists and skips safely when SDK is absent; runtime hook observability and compiled infer bridge request output exist, but compiled-hook SDK execution remains open |
...
| G13 backend failure cases covered | Partial | Backend compatibility matrix and negative tests exist; live SDK execution matrix across all backends remains open |
...
| G21-G23 deployment and observability | Partial | Docker/Kubernetes scaffolds, observability plan, runtime hook library, runtime telemetry JSON, runtime observability JSON, and compiled infer bridge request JSON exist; production OTLP SDK/exporter remains open |
| G24 governance/RFC | Partial | Release docs, candidate provenance, and PR task-stability guardrails exist; formal RFC workflow and protected release publication are open |
...
2. Compiled-code metadata/runtime lowering for `model`, `tensor`, `greenai_contract`, `greenai_measure`, and `infer`: metadata globals, runtime hook calls, runtime hook library, registry-backed hook state, source-level external runtime lowering, runtime infer observability, compiled infer bridge request JSON, and a default native-link test path exist; remaining work is passing typed tensor buffers into `AI_Runtime` for SDK-backed execution.
...
9. Runtime observability implementation with real OTLP/Prometheus export. Runtime observability JSON and compiled infer bridge request JSON exist; external exporter integration remains open.

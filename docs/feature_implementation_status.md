...
| L5 | Emit runtime metadata for AI declarations and infer in compiled code instead of no-op lowering | Partial | `IR_Generator.cpp` emits LLVM metadata globals and compiled runtime hook calls; `scripts/apply_external_runtime_to_ir_source.sh` applies guarded source-level external hook lowering; `runtime/ShorthandRuntime.cpp` provides model/tensor/contract/measurement registries with status codes; `tests/codegen/test_external_runtime_native.sh` verifies default compiler external declarations and native runtime-link execution | Default builds now compile `visitors/IR_Generator.cpp` directly after source-level runtime lowering and link native binaries through `libshorthand_runtime.a`; remaining work is routing compiled `infer` into `AI_Runtime` |
...
| C3L-5 | Resource and telemetry capture primitives | Partial | ONNX Runtime path now emits latency/input/output telemetry JSON; source-level external runtime native linking and registry-backed hook state exist; semantic IR measurement scaffolding exists; direct energy measurement and OTLP SDK export remain open |
...
| R3 | AI runtime behavior | Partial | Fallback is honest; ONNX Runtime CPU execution exists for SDK-enabled builds with an optional model-fixture gate; `libshorthand_runtime.a` exports hook symbols and maintains model/tensor/contract/measurement registries; Makefile/CMake builds now compile source-level lowered runtime hook declarations and validate native linking |
...
2. Compiled-code metadata/runtime lowering for `model`, `tensor`, `greenai_contract`, `greenai_measure`, and `infer`: metadata globals, runtime hook calls, runtime hook library, registry-backed hook state, source-level external runtime lowering, and a default native-link test path exist; remaining work is routing compiled infer into `AI_Runtime`.

...
| C3L-5 | Resource and telemetry capture primitives | Partial | ONNX Runtime path emits latency/input/output telemetry JSON; source-level external runtime native linking, registry-backed hook state, runtime infer counters, compiled-infer bridge request JSON, typed buffer bridge request JSON, and observability JSON exist; direct energy measurement and OTLP SDK export remain open |
...
| C3L-12 | CI/CD and eco-regression gates | Partial | CI exercises candidate report/check/workbook paths, C3-ECO schema/claim-safety gate, enterprise hardening checks, source diagnostics, language conformance, runtime-library build checks, source-level external runtime native linking, registry-backed runtime hook checks, runtime infer observability checks, compiled-infer bridge checks, typed buffer bridge checks, MLIR foundation checks, release supply-chain checks, and the optional ONNX SDK gate skip path; release-to-release eco-regression is still open |
...
| R3 | AI runtime behavior | Partial | Fallback is honest; ONNX Runtime CPU execution exists for SDK-enabled builds with an optional model-fixture gate; `libshorthand_runtime.a` exports hook symbols and maintains model/tensor/contract/measurement registries plus infer counters, last-infer status/reason/backend, compiled and typed buffer bridge request JSON, telemetry JSON, and observability JSON; real compiled-hook execution through `AI_Runtime` remains open |
...
| G11 real AI backend execution | Partial | Real ONNX Runtime CPU execution is implemented behind `ONNXRUNTIME_ROOT`; optional SDK gate with small ONNX fixture exists and skips safely when SDK is absent; runtime hook observability and typed buffer bridge request scaffolding exist, but compiled-hook SDK execution remains open |
...
2. Compiled-code metadata/runtime lowering for `model`, `tensor`, `greenai_contract`, `greenai_measure`, and `infer`: metadata globals, runtime hook calls, runtime hook library, registry-backed hook state, source-level external runtime lowering, runtime infer observability, a default native-link test path, compiled bridge request JSON, and typed buffer bridge request ABI exist; remaining work is routing compiled infer into `AI_Runtime` for SDK-backed execution.

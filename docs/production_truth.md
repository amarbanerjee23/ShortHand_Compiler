...
ShortHand remains `controlled_beta` with `production_claim: false` until every production blocker in the coverage and traceability matrices closes, the scoped roadmap PR96/PR99 release-candidate aggregate and applicable PR100-PR102 audit closeout gates are green, and the protected release exercise succeeds.
...
GitHub PR98 adds [realistic AI benchmark family qualification](ai_benchmark_families.md) without broadening the `linux-x64-cpu-v1` production boundary or the strict PR95 measured-energy policy. TST025 and TST026 remain partial pending controlled calibrated hardware evidence, equivalent baselines and standard task-quality evidence for the remaining families, plus remaining compiler-performance coverage. The revised plan has five delivery batches including PR98 and four afterward. Every remaining roadmap exit is preserved; the PR93 audit remains historical evidence at its original SHA.

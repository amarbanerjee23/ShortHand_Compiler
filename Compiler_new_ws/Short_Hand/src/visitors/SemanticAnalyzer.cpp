...
static bool validShape(const std::string& s) {
    bool dynamic = false;
    std::string error;
    const auto shape = shorthand::ai::parseShapeCsv(s, &dynamic, &error);
    return shorthand::ai::validateShape(shape, dynamic, &error);
}

static bool hasFallback(const std::vector<std::string>& backends) {
    return std::any_of(backends.begin(), backends.end(), [](const std::string& backend) {
        return shorthand::ai::parseBackendKind(backend) == shorthand::ai::BackendKind::Fallback;
    });
}
...
int SemanticAnalyzer::visit(AST_MODEL_DECLARATION*n){
    ...
    models[d.name]=d;
    const auto format = shorthand::ai::parseModelFormat(d.format);
    if(d.format.empty() || format == shorthand::ai::ModelFormat::Unknown) diagnostics.error("model "+d.name+" has unsupported format: "+d.format);
    if(d.input_shape.empty()||!validShape(d.input_shape)) diagnostics.error("model "+d.name+" has invalid input_shape");
    if(!d.output_shape.empty()&&!validShape(d.output_shape)) diagnostics.error("model "+d.name+" has invalid output_shape");
    if(!d.has_quality_guardrail) diagnostics.error("model "+d.name+" requires quality_guardrail");
    if(d.backend_preference.empty()) diagnostics.warning("model "+d.name+" has no backend_preference; fallback will be used if allowed");
    if(!hasFallback(d.backend_preference)) diagnostics.error("model "+d.name+" backend_preference must include fallback unless strict real-backend mode is implemented");
    for(const auto& backend_name : d.backend_preference) {
        const auto backend = shorthand::ai::parseBackendKind(backend_name);
        if(!shorthand::ai::backendSupportsFormat(backend, format)) {
            diagnostics.error("backend "+backend_name+" is incompatible with model "+d.name+" format "+d.format);
        }
    }
    const auto precision = shorthand::ai::parseElementType(d.precision);
    if(precision == shorthand::ai::ElementType::Unknown && d.precision != "fp64") diagnostics.error("model "+d.name+" has unsupported precision: "+d.precision);
    return 0;
}

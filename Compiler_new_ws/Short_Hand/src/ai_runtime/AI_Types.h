...
struct ModelSpec {
    ...
    std::string quality_op;
    double quality_threshold = 0.0;
};

struct BackendCapabilities {
    BackendKind kind = BackendKind::Fallback;
    std::string name;
    bool available = false;
    bool supports_onnx = false;
    bool supports_engine = false;
    bool supports_torchscript = false;
    bool supports_openvino_ir = false;
...
std::vector<int64_t> parseShapeCsv(const std::string& csv, bool* dynamic = nullptr, std::string* error = nullptr);
std::vector<float> parseFloatCsv(const std::string& csv, std::string* error = nullptr);
size_t productOfShape(const std::vector<int64_t>& shape);
bool validateShape(const std::vector<int64_t>& shape, bool dynamic, std::string* error = nullptr);
bool validateInputMatchesShape(const TensorSpec& spec, const TensorBuffer& input, std::string* error = nullptr);

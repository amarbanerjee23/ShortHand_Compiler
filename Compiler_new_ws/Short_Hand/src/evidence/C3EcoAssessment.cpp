...
std::vector<std::size_t> keyValuePositions(const std::string& text, const std::string& key) {
    ...
    while ((pos = text.find(needle, pos)) != std::string::npos) {
        ...
        if (colon < text.size() && text[colon] == ':') {
            positions.push_back(skipWhitespace(text, colon + 1U));
        }
        pos += needle.size();
    }
    return positions;
...
std::string typedFieldString(const std::string& text, const std::string& field) {
    const auto positions = keyValuePositions(text, field);
    require(positions.size() == 1U, "profile must contain exactly one typed field: " + field);
    const std::size_t begin = positions.front();
    const std::size_t end = std::min(text.size(), begin + 1024U);
    const std::string window = text.substr(begin, end - begin);
    const auto values = keyValuePositions(window, "value");
    require(!values.empty(), "typed profile field has no value: " + field);
    return parseJsonStringAt(window, values.front());
}

double typedFieldNumber(const std::string& text, const std::string& field) {
    const auto positions = keyValuePositions(text, field);
    require(positions.size() == 1U, "profile must contain exactly one typed field: " + field);
    const std::size_t begin = positions.front();
    const std::size_t end = std::min(text.size(), begin + 1024U);
    ...
    const auto values = keyValuePositions(window, "value");
    require(!values.empty(), "typed profile field has no value: " + field);
    return parseJsonNumberAt(window, values.front());
}
...
std::set<char> approvedNaDomains(const AssessmentInput& input) {
    const std::string value = control(input, "approved_na_domains").value;
    if (value == "none") return {};
    std::set<char> out;
    for (const std::string& tokenRaw : split(value, ',')) {
...
std::string jsonEscape(const std::string& value) {
    ...
    for (char c : value) {
        switch (c) {
            ...
            case '\t': out += "\\t"; break;
            default:
                if (static_cast<unsigned char>(c) < 0x20U) throw std::runtime_error("control character cannot be emitted in JSON");
                out.push_back(c);
        }
    }
    return out;
...
AssessmentInput loadAssessment(const std::string& path) {
    ...
    while (std::getline(in, line)) {
        ...
        for (const std::string& field : fields) require(field.find('\0') == std::string::npos, "NUL in assessment field");
        ...
        if (kind == "gate") {
            ...
            Gate gate;
            gate.declared_pass = value == "pass";
            gate.effective_pass = gate.declared_pass;
            gate.evidence_status = evidenceStatus;
            gate.evidence_ref = evidenceRef;
            input.gates.emplace(id, gate);
        } else if (kind == "criterion") {
            const auto expected = expectedCriteria();
            require(expected.count(id) == 1U, "unknown criterion id: " + id);
    ...
    const auto expected = expectedCriteria();
    require(input.criteria.size() == expected.size(), "assessment must contain the complete 76-criterion A-K catalog");
    for (const std::string& id : expected) require(input.criteria.count(id) == 1U, "assessment missing criterion: " + id);
...
ProfileInfo validateProfile(const std::string& path) {
    ...
    require(text.find("\"kind\":\"functional_unit\"") != std::string::npos, "profile evidence lacks functional_unit declaration");
    ...
    ProfileInfo info;
    info.software_class = typedFieldString(text, "software_class");
    info.functional_unit_denominator = typedFieldNumber(text, "denominator");
...
MeasurementInfo validateMeasurement(const std::string& path) {
    ...
    info.facility_energy_kwh = uniqueJsonNumber(text, "facility_energy_kwh");
    info.carbon_kgco2e = uniqueJsonNumber(text, "carbon_kgco2e");
    const double uncertaintyKwh = uniqueJsonNumber(text, "uncertainty_kwh");
...
std::map<char, DomainScore> calculateDomains(const AssessmentInput& input, bool aiMl) {
    ...
    for (const auto& domainEntry : kCriterionCounts) {
        ...
        if (domainId == 'G' && !aiMl)
            require(score.applicable_count == 0, "AI domain G must be not applicable only when the typed profile has no AI subsystem");
    ...
    require(activeBaseWeight > 0.0, "all scoring domains cannot be not applicable");
...
Decision decide(const AssessmentInput& input, const ProfileInfo& profile,
    ...
    decision.effective_gates["G14"] = decision.effective_gates["G14"] && largestOmission < 1.0 - kEpsilon && cumulativeOmission <= 5.0 + kEpsilon;

...
Current maturity: `controlled_beta`. Production claim: `false`. Active language: beta-0.6, with the beta-0.5 `shorthand.control_flow.v1` executable subset plus `shorthand.enterprise_language.v1`, offline package/lock v2 and core FFI ABI 1.0.0. The only qualified backend scope is `linux-x64-cpu-v1` with live ONNX Runtime CPU numerical execution. See `docs/production_truth.tsv` for the machine-readable authority.

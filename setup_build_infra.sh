...
# Production-supported ONNX Runtime CPU qualification is mandatory in the
# Linux x86_64 CI lane. Provision the pinned SDK once at setup time so every
...
if [[ "${CI:-}" == "true" && "${UNAME_S}/${UNAME_M}" == "Linux/x86_64" ]]; then
  ...
  test -s "${ORT_ROOT}/lib/libonnxruntime.so"
  export ONNXRUNTIME_ROOT="${ORT_ROOT}"
  export LD_LIBRARY_PATH="${ORT_ROOT}/lib:${LD_LIBRARY_PATH:-}"
  if [[ -n "${GITHUB_ENV:-}" ]]; then
    printf 'ONNXRUNTIME_ROOT=%s\n' "${ONNXRUNTIME_ROOT}" >> "${GITHUB_ENV}"
...
if [[ -n "${ONNXRUNTIME_ROOT:-}" ]]; then
  printf 'export ONNXRUNTIME_ROOT="%s"\n' "${ONNXRUNTIME_ROOT}" >> "${ROOT_DIR}/shorthand_env.sh"
  printf 'export LD_LIBRARY_PATH="%s"\n' "${LD_LIBRARY_PATH:-${ONNXRUNTIME_ROOT}/lib}" >> "${ROOT_DIR}/shorthand_env.sh"
fi
...
if [[ -n "${ONNXRUNTIME_ROOT:-}" ]]; then
  echo "ONNX Runtime enabled from ${ONNXRUNTIME_ROOT}; building ai_app for live SDK qualification."
